...
class Solution:
    ...
    def isValid(self, s: str) -> bool:
        ...
        for c in s:
            if c in '([{':
                stack.append(c)
            elif len(stack) == 0 or c != mark[stack.pop()]:
                return False
        ...
        #     if i in mark:
        #         stack.append(i)
        #     elif len(stack) > 0 and mark[stack[-1]] == i:
        #         stack.pop()
        #     else:
        #         return False
        # if len(stack) == 0:
    ...
    def findOrder(self, numCourses: int, prerequisites: list[list[int]]) -> list[int]:
        ...
        for level_list in prerequisites:
            if level_list[-1] not in courses_map:
                courses_map[level_list[-1]] = [level_list[0]]
            else:
                courses_map[level_list[-1]].append(level_list[0])

            before.add(level_list[-1])
            after.add(level_list[0])
    ...
    def peak(self, nums):
        ...
        for inx, num in enumerate(nums):
            if len(nums)-1 > inx and num > nums[inx + 1]:
                return inx
            else:
                number = num
        return inx
    ...
    # def coinChange(self, coins: list[int], amount: int) -> int:
        ...
        # else:
        #     coins.remove(maxi)
        #     return 0 + self.coinChange(coins, amount)
        #
        # if amount >= maxi:
        #     return 1 + self.coinChange(coins, amount)
        # elif amount < maxi:
        #     coins.remove(maxi)
        #     return 1 + self.coinChange(coins, amount)
        #
    ...
    def coinChange(self, coins: list[int], amount: int) -> int:
        ...
        coins.sort()
        print(f"coins.sort() - {coins}")

        for a in range(1, amount + 1):
            for c in coins:
                print(f"if {a} - {c} >= 0")
                if a - c >= 0:
                    print(f"min({dp[a]}, {1+dp[a-c]})")
        ...
        print(f"return {dp[amount]} if {dp[amount]} != {amount + 1} else -1\n")
        return dp[amount] if dp[amount] != amount + 1 else -1

    def rotate_left(self, array, N):
        """
        This function rotates a given array N times to the left.

        Args:
          array: The array to be rotated.
          N: The number of times to rotate the array.
...
if __name__ == '__main__':
    ...
    # print(a_time.findOrder(4, [[1, 0], [2, 0], [3, 1], [3, 2]]))
    # print(a_time.findOrder(6, [[3, 1], [2, 3], [2, 4], [6, 2], [5, 6], [5, 3]]))
    # print(a_time.peak([1,2,1,3,5,6,4]))
    # print(a_time.peak([1,2,3,1]))
    print(a_time.coinChange([1], 7))
    # print(a_time.rotate_left([1, 2, 3, 4, 5], 6))
